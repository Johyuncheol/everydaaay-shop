...
/* import AlertModalCard from "../../../../../shared/components/modalCard/AlertModalCard";
...
/*   const AlertModal = useModal({ isOpen: false });
  ...
  const AddToShoppingBag = () => {
    // 로그인 안되어있으먄 로그인 창으로
    if (userInfo.name === null) return navigate("/auth/login");
    ...
    if (shoppingBagData !== null) {
      ...
      putInShoppingBagAPI(JSON.stringify(newArray));
    } else {
      sessionStorage.setItem("shoppingBag", JSON.stringify(selectedItems));
      putInShoppingBagAPI(JSON.stringify(selectedItems));
    }
