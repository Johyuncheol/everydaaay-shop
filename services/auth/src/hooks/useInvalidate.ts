...
export const useInavalidate = () => {
  ...
  //유효성검사 함수 (결과 반환) 
  const checkVaildationState = ({type,data}:{type: string, data: string}) => {
    if (type === "id") {
      let regex = new RegExp("[a-z0-9]+@[a-z]+.[a-z]{2,3}");
      const validateRes = regex.test(data);

      if (validateRes || data.length === 0) return false;
      else return true;
    }
    
    else if (type === "password") {
      if (data.length < 5 && data.length >0) return true;
      else return false;
    }
  };

  return {inputData, changeInput, Message ,checkVaildationState};
};
